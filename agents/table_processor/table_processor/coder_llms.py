...
class GPTCoder(CoderLLM):
    def query(self, model, input_text: str) -> str:
...
class CodeLlamaInstructCoder(CoderLLM):
    ...
    def get_last_checkpoint(checkpoint_dir):
        ...
        if isdir(checkpoint_dir):
            is_completed = exists(
                checkpoint_dir) and "checkpoint" in checkpoint_dir
            if is_completed:
                print("what")
    ...
    def query(self, model_name: str, input_text: str, already_loaded_model=None, adapter_path: str = None,
        ...
        if bit:
            print(f"    Model '{model_name}' is quantized with \
                  {CYAN}{bit} bits.{RESET}")
        ...
        if already_loaded_model is None:
            ...
            if adapter_path:
                adapter_path, _ = CodeLlamaInstructCoder.get_last_checkpoint(
                    adapter_path)
                print(f"    Loading {CYAN}adapter\
                      {RESET} from '{adapter_path}'")
                already_loaded_model = PeftModel.from_pretrained(
                    already_loaded_model, adapter_path, offload_folder="finetuning/lora/offload/codellama_python")
        ...
        if bool(re.search(r"CodeLlama-\d+b-Instruct-hf", model_name)):
            ...
            raise ValueError(
                f"Model name '{model_name}' is not recognized [coder_llms.py].")
        ...
        if already_loaded_model is None:
            ...
            if adapter_path != "":
                adapter_path, _ = CodeLlamaInstructCoder.get_last_checkpoint(
                    adapter_path)
                already_loaded_model = PeftModel.from_pretrained(
                    already_loaded_model, adapter_path)
...
class MagiCoder(CoderLLM):
    def query(self, model_name: str, input_text: str, already_loaded_model=None, adapter_path: str = None,
        ...
        result = generator(input_text, max_length=1024,
